...
{
    public class JoystickUIView : UIView
    {
        ...
        public override void LayoutSubviews()
        {
            ...
            if (!uiBuilt && Frame.Width > 0 && Frame.Height > 0)
            {
                BuildUI(Frame);  // Pass the view’s real frame here
                uiBuilt = true;
            }
        }
        public override UIView HitTest(CGPoint point, UIEvent uievent)
        {
            ...
            Console.WriteLine($"HitTest called on JoystickUIView. Point: {point}, Result: {result?.GetType().Name ?? "null"}");
            return result;
        }

        private void BuildUI(CGRect rect)
        {
            _name = Guid.NewGuid().ToString().Substring(0, 5); //helps with debugging
            if (Frame.Width <= 0 || Frame.Height <= 0)
            {
                Frame = new CGRect(0, 0, 150, 150); // force size for debugging
            }
            ...
            StickUIView.UserInteractionEnabled = true;
